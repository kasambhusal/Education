...
const NewPostForm = ({ isOpen, onClose, category, onPostCreated }) => {
    ...
    useEffect(() => {
        // Only reset if the form was previously mounted and is now closing
        if (!isOpen && formMounted.current) {
            // Use a small timeout to ensure the form is still mounted when we reset
            const timer = setTimeout(() => {
                if (form) {
                    try {
                        form.resetFields()
                    } catch (error) {
                        console.log("Form reset skipped - form may be unmounted")
                    }
                }
                setImageUrl("")
                setImageError(false)
